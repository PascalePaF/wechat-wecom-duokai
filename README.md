...
![V1.0.7 日间主题最小界面](docs/assets/v1.0.7-main-light-901x513.png)

## V1.0.7 有什么变化

- 设置页新增由用户确认的“一键更新”：安装版和绿色版都能原地更新并保留 `data`。
- 只接受本项目 GitHub 正式 Release 中名称、版本、大小和 URL 全部符合预期的 setup 附件。
- GitHub 服务端附件摘要、`SHA256SUMS.txt` 和下载文件三方 SHA-256 必须完全一致；任何缺失或冲突都会停止。
- 从最终哈希计算到启动安装程序之间锁定下载文件不可写；安装程序还会再次核对自身哈希、更新计划、
  父进程、版本和目标目录标记。
- 新程序文件先释放到 `data\updates` 的同卷暂存区，替换时逐项备份；中途失败会逆序恢复旧文件。
- 未经确认不会下载或安装；检查失败、离线或附件不合规都不影响微信/企业微信多开功能。
- 更新结果仅写入本地 `data\logs\update.log`，不会上传诊断、路径、账号或其他本机数据。
- 新增自动化用例模拟第二个文件替换后中断，验证旧程序完整回滚且设置文件保持不变。
- 目标窗口数恢复为微信与企业微信共用的单一数字；升级时会自动合并 V1.0.6 的两个旧配置。

完整记录见 [CHANGELOG.md](CHANGELOG.md)。下载信任链、目录限制、事务回滚和测试证据见
[V1.0.7 一键更新安全验证报告](docs/V1.0.7-一键更新安全验证报告.md)。
...
| 设置中心 | 跟随系统/日间/夜间主题，启动检查开关、手动检查和确认式一键更新 |
...
| 文件 | 适用场景 |
| --- | --- |
| `wechat_duokai-setup-v1.0.7.exe` | 推荐；可选择本机安装地址，也是经过校验的一键更新执行程序 |
| `wechat_duokai-portable-v1.0.7.zip` | 绿色版；解压完整目录后直接运行，也支持保持绿色身份原地更新 |
| `wechat_duokai-cleanup-v1.0.7.exe` | 独立完整卸载与清理工具；与安装器不是同一个二进制 |
| `SHA256SUMS.txt` | 核对下载内容是否与发布文件一致 |
| `manifest.spdx.json` | Microsoft SBOM Tool 生成并验证的 SPDX 2.2 软件物料清单 |
| `kaspersky-scan-v1.0.7.txt` | Kaspersky 对正式发布附件原件的原始扫描日志 |
...
1. 下载并运行 `wechat_duokai-setup-v1.0.7.exe`。
...
![V1.0.7 日间主题安装器](docs/assets/v1.0.7-installer-light.png)

## 绿色版

1. 将 `wechat_duokai-portable-v1.0.7.zip` 解压到一个独立文件夹。
...
账号、聊天内容或凭据。V1.0.7 会把 V1.0.6 分开保存的两个数量合并为一个，优先沿用微信的旧值；
...
![V1.0.7 设置页](docs/assets/v1.0.7-settings-light-901x513.png)

<details>
<summary>查看夜间主题与大尺寸适配</summary>

![V1.0.7 夜间主题最小界面](docs/assets/v1.0.7-main-dark-901x513.png)

![V1.0.7 1920×1080 夜间界面](docs/assets/v1.0.7-main-dark-1920x1080.png)

![V1.0.7 3440×1392 超宽屏界面](docs/assets/v1.0.7-main-ultrawide-light-3440x1392.png)
...
- [V1.0.7 完整安全审计报告](docs/微信企业微信多开助手_V1.0.7_完整安全审计报告.txt)
- [V1.0.7 一键更新安全验证报告](docs/V1.0.7-一键更新安全验证报告.md)
- [V1.0.7 UI 回归矩阵](docs/V1.0.7-UI回归矩阵.md)
...
![V1.0.7 完整卸载器](docs/assets/v1.0.7-uninstaller-light.png)
...
发布目录为 `artifacts\V1.0.7`。脚本分别复制真正的 setup 与 cleanup 输出，并生成绿色版、发布清单
...
- V1.0.7 沿用真机基线：微信 `4.1.15.6`、企业微信 `5.0.11.6018`、Windows 11 x64，二者腾讯签名有效；
...
- V1.0.7 继续使用 WPF/.NET Framework 4.8；现有矢量框架和独立安装器可以实现安全更新，无需迁移
