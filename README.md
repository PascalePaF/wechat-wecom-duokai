...
![V1.1.1 日间主题最小界面](docs/assets/v1.1.0-main-light-901x513.png)

## V1.1.1 有什么变化

- 修复共享网络耗尽 GitHub 未登录 API 每小时 60 次配额后，“检查更新”错误显示失败的问题。
- 最新版号先从 GitHub 普通 Release 跳转读取，不消耗 API 配额；只有发现新版本时才尝试取得附件摘要。
- API 配额耗尽或暂时不可用时，自动改用固定的本项目 HTTPS Release 附件、严格大小限制、
  `SHA256SUMS.txt` 与下载文件 SHA-256 校验，仍可安全一键更新。
- 网络真的不可用时，设置页会显示超时、连接失败或版本地址异常等具体原因，不再隐藏成统一提示。
- 新增 API 403/配额为 0 的自动回归测试，并在本机真实限流状态下验证仍能发现 V1.1.0 及其正式附件。
- V1.1.0 的分端全部退出、界面对齐、数量强调以及全部多开与恢复边界保持不变。

完整记录见 [CHANGELOG.md](CHANGELOG.md)。本次根因、回归和安全边界见
[V1.1.1 一键更新安全验证报告](docs/V1.1.1-一键更新安全验证报告.md)与
[V1.1.1 完整安全审计报告](docs/微窗助手_V1.1.1_完整安全审计报告.txt)；界面未改版，继续沿用
[V1.1.0 UI 回归矩阵](docs/V1.1.0-UI回归矩阵.md)。
...
| 一键更新 | 不受公共 API 配额阻断；固定 GitHub Release、SHA-256、独立安装进程与失败回滚 |
...
| 文件 | 适用场景 |
| --- | --- |
| `wechat_duokai-setup-v1.1.1.exe` | 推荐；可选择本机安装地址，也是经过校验的一键更新执行程序 |
| `wechat_duokai-portable-v1.1.1.zip` | 绿色版；解压完整目录后直接运行，也支持保持绿色身份原地更新 |
| `wechat_duokai-cleanup-v1.1.1.exe` | 独立完整卸载与清理工具；与安装器不是同一个二进制 |
| `SHA256SUMS.txt` | 核对下载内容是否与发布文件一致 |
| `manifest.spdx.json` | Microsoft SBOM Tool 生成并验证的 SPDX 2.2 软件物料清单 |
| `kaspersky-scan-v1.1.1.txt` | Kaspersky 对正式发布附件原件的原始扫描日志 |
...
1. 下载并运行 `wechat_duokai-setup-v1.1.1.exe`。
...
![V1.1.1 日间主题安装器](docs/assets/v1.1.0-installer-light.png)

## 绿色版

1. 将 `wechat_duokai-portable-v1.1.1.zip` 解压到一个独立文件夹。
...
账号、聊天内容或凭据。V1.1.1 延续 V1.0.7 的迁移规则：把 V1.0.6 分开保存的两个数量合并为一个，
优先沿用微信的旧值；
...
版本检查先访问本项目 GitHub `releases/latest` 跳转，不消耗未登录 API 的每小时 60 次配额。只有发现新版本
时才尝试读取 API 附件摘要；配额耗尽时自动使用精确版本的固定 HTTPS Release 地址与附件大小元数据。
点击一键更新后还会显示确认框；只有确认后才下载 `SHA256SUMS.txt` 和 setup。下载内容全部进入当前程序
目录的 `data\updates`。能取得 GitHub 附件摘要时执行摘要、清单、下载文件三方校验；API 暂不可用时仍强制
固定地址、大小上限、清单与下载文件 SHA-256 一致。安装版会刷新登记，绿色版保持绿色身份；失败时回滚。
...
├─ ReleaseUpdateChecker           配额无关的 Release 发现、严格 URL/大小与可选 GitHub digest
├─ ApplicationUpdateService       Release 清单/SHA-256、受限下载目录、计划落盘与独立安装器交接
...
- 诊断报告仅写本地，不自动上传。
- 用户启用启动检查时，自动网络请求先向 `github.com` 读取本项目最新正式 Release 跳转；仅在发现新版本后
  尝试向 `api.github.com` 取得附件摘要，API 配额不可用会自动安全降级而不会误报失败。可关闭自动检查。
  只有用户确认一键更新后，才下载同一 Release 的 `SHA256SUMS.txt` 与精确版本 setup。程序不上传诊断、
  路径、账号或日志；GitHub 会看到普通 HTTPS 请求所需的 IP 和 User-Agent。
- 自动覆盖只接受专用标记目录。能取得 GitHub digest 时执行 digest、SHA256SUMS 与本机文件三方校验；
  API 不可用时仍要求固定仓库/标签/附件路径、HTTPS、大小限制、SHA256SUMS 与下载文件一致。替换使用
...
- [V1.1.1 完整安全审计报告](docs/微窗助手_V1.1.1_完整安全审计报告.txt)
- [V1.1.1 一键更新安全验证报告](docs/V1.1.1-一键更新安全验证报告.md)
- [V1.1.0 全项目自查报告](docs/V1.1.0-全项目自查报告.md)
- [V1.1.0 UI 回归矩阵](docs/V1.1.0-UI回归矩阵.md)
...
![V1.1.1 完整卸载器](docs/assets/v1.1.0-uninstaller-light.png)
...
PowerShell -ExecutionPolicy Bypass -File .\build-release.ps1 -Version 1.1.1
```

发布目录为 `artifacts\V1.1.1`。脚本分别复制真正的 setup 与 cleanup 输出，并生成绿色版、发布清单
...
- V1.1.1 沿用真机基线：微信 `4.1.15.6`、企业微信 `5.0.11.6018`、Windows 11 x64，二者腾讯签名有效；
...
- V1.1.1 继续使用 WPF/.NET Framework 4.8；现有矢量框架和独立安装器可以实现主题动态资源与安全更新，无需迁移
