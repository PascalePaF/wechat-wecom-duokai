...
![V1.1.0 日间主题最小界面](docs/assets/v1.1.0-main-light-901x513.png)

## V1.1.0 有什么变化

- 微信和企业微信卡片各加入一个紧凑的“全部退出”按钮，不新增独立面板，也不破坏原有启动布局。
- 用户确认后，只处理当前 Windows 会话中路径与已验证官方客户端完全一致的进程；先请求正常关闭，超时后才处理残留进程。
- 左右两块主卡片统一为同一套比例行结构，标题、控件和外框在最小窗口到全屏状态都保持对齐。
- 左下角实时数量改为带留白的粗体数字：微信使用绿色、企业微信使用蓝色，更容易快速辨认。
- 移除品牌名下方重复的“微信 · 企业微信”副标题，并压缩目标数量说明，主界面更简洁。
- 新增退出边界、按钮状态、数字强调和布局对齐自动测试，继续保留三方哈希更新、SBOM 与安全扫描门禁。

完整记录见 [CHANGELOG.md](CHANGELOG.md)。安全更新链、目录限制、主题回归和测试证据见
[V1.1.0 全项目自查报告](docs/V1.1.0-全项目自查报告.md)、
[V1.1.0 一键更新安全验证报告](docs/V1.1.0-一键更新安全验证报告.md)与
...
| 文件 | 适用场景 |
| --- | --- |
| `wechat_duokai-setup-v1.1.0.exe` | 推荐；可选择本机安装地址，也是经过校验的一键更新执行程序 |
| `wechat_duokai-portable-v1.1.0.zip` | 绿色版；解压完整目录后直接运行，也支持保持绿色身份原地更新 |
| `wechat_duokai-cleanup-v1.1.0.exe` | 独立完整卸载与清理工具；与安装器不是同一个二进制 |
| `SHA256SUMS.txt` | 核对下载内容是否与发布文件一致 |
| `manifest.spdx.json` | Microsoft SBOM Tool 生成并验证的 SPDX 2.2 软件物料清单 |
| `kaspersky-scan-v1.1.0.txt` | Kaspersky 对正式发布附件原件的原始扫描日志 |
...
1. 下载并运行 `wechat_duokai-setup-v1.1.0.exe`。
...
![V1.1.0 日间主题安装器](docs/assets/v1.1.0-installer-light.png)

## 绿色版

1. 将 `wechat_duokai-portable-v1.1.0.zip` 解压到一个独立文件夹。
...
账号、聊天内容或凭据。V1.1.0 延续 V1.0.7 的迁移规则：把 V1.0.6 分开保存的两个数量合并为一个，
优先沿用微信的旧值；
...
- [V1.1.0 完整安全审计报告](docs/微窗助手_V1.1.0_完整安全审计报告.txt)
- [V1.1.0 全项目自查报告](docs/V1.1.0-全项目自查报告.md)
- [V1.1.0 一键更新安全验证报告](docs/V1.1.0-一键更新安全验证报告.md)
- [V1.1.0 UI 回归矩阵](docs/V1.1.0-UI回归矩阵.md)
...
![V1.1.0 完整卸载器](docs/assets/v1.1.0-uninstaller-light.png)
...
发布目录为 `artifacts\V1.1.0`。脚本分别复制真正的 setup 与 cleanup 输出，并生成绿色版、发布清单
...
- V1.1.0 沿用真机基线：微信 `4.1.15.6`、企业微信 `5.0.11.6018`、Windows 11 x64，二者腾讯签名有效；
...
- V1.1.0 继续使用 WPF/.NET Framework 4.8；现有矢量框架和独立安装器可以实现主题动态资源与安全更新，无需迁移
